...
import '../../controllers/AdsManageController.dart' show ManageAdController;
...
class _FilterScreenState extends State<FilterScreen> {
  ...
  // ✅ المصدر الحقيقي الآن للتصنيفات الفرعية (Level 1/2)
  final ManageAdController _manage = Get.isRegistered<ManageAdController>()
      ? Get.find<ManageAdController>()
      : Get.put(ManageAdController());
  ...
  // ✅ Snapshots للتصنيفات الفرعية (IDs)
  int? _tempSub1Id;
  int? _tempSub2Id;
  ...
  int? _cleanId(int? v) => (v != null && v > 0) ? v : null;

  /// اسم آمن لأي موديل (Category/Subcategory..)
  String _nameFromModel(Object item) {
    final dynamic d = item;

    // translations?.first?.name
    try {
      final t = d.translations;
      if (t is List && t.isNotEmpty) {
        final n = t.first.name;
  ...
  SubcategoryLevelOne? _findSub1ById(int id) {
    try {
      return _manage.subCategories.firstWhere((x) => x.id == id);
    } catch (_) {
      return null;
    }
  }

  SubcategoryLevelTwo? _findSub2ById(int id) {
    try {
      return _manage.subCategoriesLevelTwo.firstWhere((x) => x.id == id);
    } catch (_) {
      return null;
    }
  }

  void _syncAdsListsFromManage() {
    // مزامنة القوائم داخل AdsController (احتياط عشان باقي الشاشات/المنطق)
    _ads.subCategoryLevelOneList.assignAll(_manage.subCategories);
    _ads.subCategoryLevelTwoList.assignAll(_manage.subCategoriesLevelTwo);
  }

  void _clearSubCategories({bool clearLists = true}) {
    // ✅ ManageAdController
    _manage.selectedSubcategoryLevelOne.value = null;
    _manage.selectedSubcategoryLevelTwo.value = null;

    // ✅ AdsController (حتى ما ينكسر أي مكان يعتمد عليه)
    _ads.currentSubCategoryLevelOneId.value = null;
    _ads.currentSubCategoryLevelTwoId.value = null;

    if (clearLists) {
      _manage.subCategories.clear();
      _manage.subCategoriesLevelTwo.clear();

      _ads.subCategoryLevelOneList.clear();
      _ads.subCategoryLevelTwoList.clear();
    }
  }

  /// ✅ تحميل subcats مع الحفاظ على الاختيارات السابقة (إن كانت صالحة)
  Future<void> _loadSubsForMain(int mainCategoryId, String lang) async {
    // الأولوية للـ Manage selections، ولو فاضية خذ من Ads IDs (للتوافق)
    final prevSub1 = _cleanId(
      _manage.selectedSubcategoryLevelOne.value?.id ??
          _ads.currentSubCategoryLevelOneId.value,
    );
    final prevSub2 = _cleanId(
      _manage.selectedSubcategoryLevelTwo.value?.id ??
          _ads.currentSubCategoryLevelTwoId.value,
    );
    ...
    await _manage.fetchSubcategories(mainCategoryId, lang);
    _syncAdsListsFromManage();

    // تحقق من صلاحية prevSub1
    final bool sub1Exists =
        prevSub1 != null && _manage.subCategories.any((x) => x.id == prevSub1);
    ...
    // رجّع sub1
    final sub1Model = _findSub1ById(prevSub1!);
    _manage.selectedSubcategoryLevelOne.value = sub1Model;

    _ads.currentSubCategoryLevelOneId.value = prevSub1;
    _ads.currentSubCategoryLevelTwoId.value = null;

    // حمّل Level 2 بناءً على sub1
    await _manage.fetchSubcategoriesLevelTwo(prevSub1, lang);
    _syncAdsListsFromManage();

    final bool sub2Exists =
        prevSub2 != null && _manage.subCategoriesLevelTwo.any((x) => x.id == prevSub2);

    if (sub2Exists) {
      final sub2Model = _findSub2ById(prevSub2!);
      _manage.selectedSubcategoryLevelTwo.value = sub2Model;
      _ads.currentSubCategoryLevelTwoId.value = prevSub2;
    } else {
      _manage.selectedSubcategoryLevelTwo.value = null;
      _ads.currentSubCategoryLevelTwoId.value = null;
    }
  ...
  Future<void> _onSubLevelOneChanged(int? sub1Id) async {
    final lang = Get.find<ChangeLanguageController>()
        .currentLocale
        .value
        .languageCode;

    final cleaned = _cleanId(sub1Id);

    // تحديث الـ Ads IDs للتوافق
    _ads.currentSubCategoryLevelOneId.value = cleaned;
    _ads.currentSubCategoryLevelTwoId.value = null;
    _ads.subCategoryLevelTwoList.clear();

    // تحديث Manage selections
    _manage.selectedSubcategoryLevelOne.value =
        cleaned == null ? null : _findSub1ById(cleaned);
    _manage.selectedSubcategoryLevelTwo.value = null;
    _manage.subCategoriesLevelTwo.clear();

    if (cleaned != null) {
      await _manage.fetchSubcategoriesLevelTwo(cleaned, lang);
      _syncAdsListsFromManage();
    }
  ...
  void _onSubLevelTwoChanged(int? sub2Id) {
    final cleaned = _cleanId(sub2Id);

    // Ads IDs
    _ads.currentSubCategoryLevelTwoId.value = cleaned;

    // Manage selection
    _manage.selectedSubcategoryLevelTwo.value =
        cleaned == null ? null : _findSub2ById(cleaned);
  ...
  void initState() {
    ...
    // ✅ Subcategories snapshots (من Manage)
    _tempSub1Id = _manage.selectedSubcategoryLevelOne.value?.id ??
        _ads.currentSubCategoryLevelOneId.value;
    _tempSub2Id = _manage.selectedSubcategoryLevelTwo.value?.id ??
        _ads.currentSubCategoryLevelTwoId.value;
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      // التصنيفات الرئيسية (لو غير ثابت)
      ...
      if (eff != null) {
        // ✅ حمّل (Subcategories + Attributes) معاً
        await Future.wait([
          _ads.fetchAttributes(categoryId: eff, lang: lang),
          _loadSubsForMain(eff, lang), // ✅ من ManageAdController
        ]);
        _pruneAttributeValuesToLoadedAttributes();
      } else {
        _ads.resetAttributesState();
        _clearSubCategories(clearLists: true);
      }
  ...
  void dispose() {
    if (!_didApply) {
      ...
      // ✅ رجّع IDs داخل AdsController
      _ads.currentSubCategoryLevelOneId.value = _tempSub1Id;
      _ads.currentSubCategoryLevelTwoId.value = _tempSub2Id;

      // ✅ رجّع داخل ManageAdController قدر الإمكان (حسب القوائم الحالية)
      if (_tempSub1Id != null) {
        _manage.selectedSubcategoryLevelOne.value = _findSub1ById(_tempSub1Id!) ??
            _manage.selectedSubcategoryLevelOne.value;
      } else {
        _manage.selectedSubcategoryLevelOne.value = null;
      }
      ...
      if (_tempSub2Id != null) {
        _manage.selectedSubcategoryLevelTwo.value = _findSub2ById(_tempSub2Id!) ??
            _manage.selectedSubcategoryLevelTwo.value;
      } else {
        _manage.selectedSubcategoryLevelTwo.value = null;
      }
      ...
      for (final item in list) {
        ...
        if (type == 'boolean') {
          if (value is bool) {
            ...
            if (s.isNotEmpty) {
              _attributeValues[aid] =
                  (s == 'true' || s == '1' || s == 'yes' || s == 'نعم');
            }
          }
          continue;
        }
  ...
  Future<void> _onMainCategoryChanged(int? val) async {
    ...
    if (val != null && val > 0) {
      await Future.wait([
        _ads.fetchAttributes(categoryId: val, lang: lang),
        _manage.fetchSubcategories(val, lang), // ✅ Level 1 من ManageAdController
      ]);

      _syncAdsListsFromManage(); // ✅ مزامنة
      _pruneAttributeValuesToLoadedAttributes();
      if (mounted) setState(() {});
    }
  }
          ...
          child: KeyedSubtree(
            ...
            child: SingleChildScrollView(
              padding: EdgeInsets.only(
                  left: 16.w, right: 16.w, top: 8.h, bottom: 120.h),
              child: Column(
                ...
                children: [
                  ...
                  // ✅ التصنيفات الفرعية (مستوى 1 و 2) من ManageAdController
  ...
  Widget _buildSubCategoriesSection() {
    ...
    return Obx(() {
      final loading1 = _manage.isLoadingSubcategoryLevelOne.value;
      final loading2 = _manage.isLoadingSubcategoryLevelTwo.value;

      final List<SubcategoryLevelOne> sub1List =
          _manage.subCategories.toList();
      final List<SubcategoryLevelTwo> sub2List =
          _manage.subCategoriesLevelTwo.toList();

      // لو التصنيف ما له فروع… اعرض رسالة بدل ما تختفي
      if (!loading1 && sub1List.isEmpty) {
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _buildSectionTitle('التصنيف الفرعي'.tr),
            _noteRow('لا توجد تصنيفات فرعية لهذا التصنيف.'.tr),
            SizedBox(height: 10.h),
          ],
        );
      }

      final sub1Id = _cleanId(_manage.selectedSubcategoryLevelOne.value?.id);
      final safeSub1 =
          (sub1Id != null && sub1List.any((x) => x.id == sub1Id)) ? sub1Id : null;

      final sub2Id = _cleanId(_manage.selectedSubcategoryLevelTwo.value?.id);
      final safeSub2 =
          (sub2Id != null && sub2List.any((x) => x.id == sub2Id)) ? sub2Id : null;
        ...
        children: [
          ...
          if (loading1)
            ...
            DropdownButtonFormField<int>(
              ...
              decoration: _inputDecoration(hint: 'اختر تصنيفًا فرعيًا'.tr),
              items: sub1List
                  .map(
                  ...
                  .toList(),
              onChanged: (v) => _onSubLevelOneChanged(v),
          ...
          if (safeSub1 != null) ...[
            SizedBox(height: 8.h),
            if (loading2)
              _noteRow('جارٍ تحميل التصنيف الفرعي الثاني...'.tr)
            else if (sub2List.isNotEmpty)
              DropdownButtonFormField<int>(
                value: safeSub2,
                isExpanded: true,
                menuMaxHeight: MediaQuery.of(context).size.height * 0.5,
                decoration:
                    _inputDecoration(hint: 'اختر تصنيفًا فرعيًا ثانيًا'.tr),
                items: sub2List
                    .map(
                      (s) => DropdownMenuItem<int>(
                        value: s.id,
                    ...
                    .toList(),
                onChanged: (v) => _onSubLevelTwoChanged(v),
                dropdownColor: AppColors.card(isDark),
              )
            else
              _noteRow('لا يوجد تصنيف فرعي ثاني لهذا الاختيار.'.tr),
          ],
                ...
                if (attribute.type == 'options' && attribute.isMultiSelect)
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                          color: AppColors.border(isDark), width: 0.6),
                    ),
                    child: Text(
                      'متعدد'.tr,
                      style: TextStyle(
  ...
  Widget _buildMultiOptionsAttribute(CategoryAttribute attribute) {
    ...
    final selectedOptions =
        attribute.options.where((o) => selectedIds.contains(o.id)).toList();
                ...
                Expanded(
                  child: Text(
                    ...
                    style: TextStyle(
                      ...
                      fontWeight:
                          selectedIds.isEmpty ? FontWeight.w500 : FontWeight.w600,
                    ),
                  ),
                ),
        ...
        return StatefulBuilder(
          builder: (ctx, setSheetState) {
            final filtered = query.trim().isEmpty
                ...
                : all
                    .where((o) =>
                        o.value.toLowerCase().contains(query.toLowerCase()))
                    .toList();
  ...
  Widget _buildBooleanAttribute(CategoryAttribute attribute) {
    ...
    return Row(
      children: [
        _boolChip('نعم'.tr, current == true,
            () => setState(() => _attributeValues[attribute.attributeId] = true)),
        SizedBox(width: 12.w),
        _boolChip('لا'.tr, current == false,
  ...
  void _onResetPressed() {
    ...
    () async {
      ...
      if (widget.categoryId == null || (widget.categoryId ?? 0) <= 0) {
        ...
        final fixed = widget.categoryId!;
        await Future.wait([
          _ads.fetchAttributes(categoryId: fixed, lang: lang),
          _manage.fetchSubcategories(fixed, lang), // ✅ Level 1 من ManageAdController
        ]);
  ...
  Future<void> _applyFilters() async {
    ...
    final priceMax =
        _priceMode == PriceMode.minOnly ? null : _parsePrice(_priceMaxCtrl.text);
    ...
    // ✅ sub1/sub2 من ManageAdController (مع fallback لـ Ads IDs)
    final sub1 = _cleanId(_manage.selectedSubcategoryLevelOne.value?.id ??
        _ads.currentSubCategoryLevelOneId.value);
    final sub2 = _cleanId(_manage.selectedSubcategoryLevelTwo.value?.id ??
        _ads.currentSubCategoryLevelTwoId.value);
    ...
    try {
      ...
      Get.snackbar('خطأ'.tr, 'فشل الاتصال: $e',
          snackPosition: SnackPosition.BOTTOM);
    } finally {
